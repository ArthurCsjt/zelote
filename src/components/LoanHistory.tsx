...
import { Clock, Monitor, User, CheckCircle, AlertTriangle, Search, X, RotateCcw, CalendarX, List, LayoutGrid, ArrowRight, ArrowDown } from "lucide-react";
        ...
        <div className="space-y-2">
          ...
          {paginatedHistory.map((loan) => {
            ...
            return (
              <div
                key={loan.id}
                className="grid grid-cols-1 md:grid-cols-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)] transition-all duration-200 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:hover:shadow-[6px_6px_0px_0px_rgba(255,255,255,0.3)] hover:-translate-x-0.5 hover:-translate-y-0.5"
              >

                {/* ══ LEFT SIDE: LOAN ══ */}
                <div className={cn(
                  "border-3 border-black dark:border-white md:border-r-0 p-4 transition-all duration-300",
                  isReturned
                    ? "bg-zinc-100 dark:bg-zinc-900/60 border-l-[6px] border-l-green-500"
                    : cn("border-l-[6px]", cardClass, "bg-card")
                )}>
                  {/* Mobile Label */}
                  <div className="md:hidden flex items-center gap-1.5 mb-3 text-muted-foreground/60 border-b pb-2">
                    <CheckCircle className="h-3 w-3" />
                    <span className="text-[10px] font-black uppercase tracking-widest">Início do Fluxo</span>
                  </div>
                  {isReturned ? (
                    ...
                    <div className="flex items-center justify-between h-full min-h-[90px] gap-2">
                      <div className="flex flex-col gap-1.5 min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-black text-base uppercase tracking-tight">{loan.chromebook_id}</span>
                        </div>
                        <span className="text-sm font-bold uppercase truncate text-foreground/70">{loan.student_name}</span>
                        <span className="text-xs font-mono text-muted-foreground">
                          {format(new Date(loan.loan_date), "dd/MM/yy HH:mm")}
                        </span>
                        {loan.created_by_email && (
                          <span className="text-[10px] font-bold uppercase text-muted-foreground opacity-60">REGISTRADO POR: {loan.created_by_email}</span>
                        )}
                        <div className="flex items-center gap-1 flex-wrap mt-0.5">
                          <Badge className={cn("text-xs rounded-full h-5 px-3 border font-black", statusBadgeClass)}>
                            {loan.status}
                          </Badge>
                          {loan.loan_type === 'lote' && (
                            <Badge className="text-xs rounded-full h-5 px-3 bg-amber-100 text-amber-900 border-2 border-amber-500 font-black">LOTE</Badge>
                          )}
                        </div>
                      </div>
                      {/* Arrow pointing right — journey complete */}
                      <div className="flex flex-col items-center gap-1 shrink-0">
                        <ArrowRight className="hidden md:block h-8 w-8 text-green-500 dark:text-green-400" strokeWidth={2.5} />
                        <ArrowDown className="md:hidden h-8 w-8 text-green-500 dark:text-green-400" strokeWidth={2.5} />
                      </div>
                    </div>
                  ) : (
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h3 className="font-black text-base uppercase truncate">{loan.student_name}</h3>
                            <p className="text-xs font-bold text-muted-foreground truncate">{loan.student_email}</p>
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          <Badge className={cn("text-xs rounded-full border-2 font-black px-3 h-5", statusBadgeClass)}>
                            {loan.status}
                          </Badge>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {loan.student_ra && (
                          <Badge className="text-xs rounded-full h-5 px-3 bg-white dark:bg-zinc-900 text-black dark:text-white border-2 border-black dark:border-white font-bold uppercase">
                            RA: {loan.student_ra}
                          </Badge>
                        )}
                        <Badge className="text-xs rounded-full h-5 px-3 bg-violet-100 dark:bg-violet-900 text-violet-900 dark:text-violet-100 border-2 border-violet-600 font-bold uppercase">
                          <Monitor className="h-3 w-3 mr-1" />{loan.chromebook_id}
                        </Badge>
                        <Badge className="text-xs rounded-full h-5 px-3 bg-zinc-200 dark:bg-zinc-800 text-black dark:text-white border-2 border-black dark:border-white font-bold uppercase">
                          {loan.user_type}
                        </Badge>
                        {loan.loan_type === 'lote' && (
                          <Badge className="text-xs rounded-full h-5 px-3 bg-amber-100 dark:bg-amber-900 text-amber-900 dark:text-amber-100 border-2 border-amber-600 font-bold uppercase">LOTE</Badge>
                        )}
                        {loan.reservation_id && (
                          <Badge className="text-xs rounded-full h-5 px-3 bg-indigo-500 text-white border-2 border-indigo-700 font-bold uppercase">
                      ...
                      {loan.purpose && (
                        <div className="px-2 py-1.5 border-2 border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/50 text-xs font-black uppercase text-zinc-500 truncate">
                          {loan.purpose}
                        </div>
                      )}
                      ...
                      {/* Mobile Arrow for Active Loans */}
                      <div className="md:hidden flex flex-col items-center gap-2 pt-4 border-t border-dashed mt-4">
                        <div className="flex items-center gap-1.5 mb-1 text-muted-foreground/40">
                          <Clock className="h-3 w-3" />
                          <span className="text-[10px] font-black uppercase tracking-widest">Aguardando Retorno</span>
                        </div>
                        <ArrowDown className="h-8 w-8 text-zinc-300 dark:text-zinc-700 animate-pulse" strokeWidth={2} />
                      </div>
                    </div>
                  )}
                </div>
                ...
                {/* ══ RIGHT SIDE: RETURN ══ */}
                <div className={cn(
                  "border-3 border-black dark:border-white md:border-l-0 p-4 transition-all duration-300",
                  isReturned
                    ? cn(
                        "md:border-l-0",
                        isLateReturn
                  ...
                  <div className="md:hidden flex items-center gap-1.5 mb-3 text-muted-foreground/60 border-b pb-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Conclusão do Fluxo</span>
                  </div>
                  {isReturned && loan.return_date ? (
                    ...
                    <div className="space-y-2.5 h-full">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {isLateReturn && (
                          <Badge className="text-[10px] rounded-full bg-red-500 text-white border-2 border-red-700 font-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] px-3 h-5">FORA DO PRAZO</Badge>
                        )}
                      </div>
                      ...
                      {loan.return_notes && (
                        <div className="px-2 py-1.5 border-2 border-zinc-200 dark:border-zinc-700 bg-white/60 dark:bg-zinc-900/40 text-[11px] font-bold uppercase text-zinc-500 italic mt-1">
                          "{loan.return_notes}"
                        </div>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.25em] opacity-25">Aguardando</span>
                    </div>
                  )}
                </div>
              </div>
