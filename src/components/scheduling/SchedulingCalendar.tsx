      ...
      >
        ...
        >
          ...
          >
            ...
            <Monitor className="absolute -top-3 -right-3 h-10 w-10 text-primary/10 dark:text-primary/15 pointer-events-none stroke-[2.5] z-0 select-none rotate-12" />
                ...
                {isEditingLimit ? (
                  <div className="flex flex-col items-center gap-1 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                    <input
                      ...
                      className="w-14 h-6 text-center bg-white dark:bg-zinc-900 border-2 border-black dark:border-white text-black dark:text-white font-black text-[11px] focus:outline-none focus:ring-0"
                      autoFocus
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                    ...
                    <div className="flex gap-1">
                      <button
                        ...
                        className="bg-black dark:bg-white text-white dark:text-black text-[7px] px-1.5 py-0.5 font-black uppercase hover:opacity-85 transition-opacity"
                      >
                        Salvar
                      </button>
                      <button
                        ...
                        className="bg-white dark:bg-zinc-800 text-black dark:text-white border border-black dark:border-white text-[7px] px-1.5 py-0.5 font-black uppercase hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors"
                      >
                        X
                      </button>
                    </div>
                  ...
                  <>
                    <div className="flex flex-col items-center leading-none gap-0.5 select-none relative">
                      <span className="text-2xl sm:text-[28px] font-[1000] tabular-nums text-black dark:text-white tracking-tighter leading-none">
                        {totalAvailableChromebooks}
                      </span>
                      <span className="text-[7px] sm:text-[8px] font-black uppercase text-zinc-500 dark:text-zinc-400 tracking-[0.22em] leading-none text-center">
            ...
            {/* Bottom Half - Heatmap legend aligned with daily heatmaps */}
            <div className="px-1.5 sm:px-2 py-1.5 border-t-4 border-black dark:border-white bg-zinc-50 dark:bg-zinc-950 relative z-10 flex flex-col items-center justify-center gap-1 h-[40px] sm:h-[50px] select-none shrink-0 w-full overflow-hidden">
              <div className="flex items-center gap-1 leading-none justify-center">
                <span className="text-[6px] sm:text-[8px] font-[1000] uppercase tracking-[0.12em] text-zinc-500 dark:text-zinc-400 leading-none whitespace-nowrap">
                  Mapa de Calor
                </span>
              </div>
              {/* Discrete block legend perfectly matching daily heatmap column height */}
              <div className="flex h-[8px] sm:h-[10px] gap-[2px] relative z-20 justify-center w-[calc(100%-8px)] items-end">
                <div className="flex-1 h-full bg-zinc-200/60 dark:bg-zinc-700/30 border border-black/10 dark:border-white/10 hover:scale-y-125 transition-all duration-150 origin-bottom cursor-help rounded-[1px]" title="Totalmente Livre (0% de uso)" />
                <div className="flex-1 h-full bg-amber-300 dark:bg-amber-400 border border-black/10 dark:border-white/10 hover:scale-y-125 transition-all duration-150 origin-bottom cursor-help rounded-[1px]" title="Pouco Uso (<30% reservado)" />
                <div className="flex-1 h-full bg-orange-400 dark:bg-orange-400 border border-black/10 dark:border-white/10 hover:scale-y-125 transition-all duration-150 origin-bottom cursor-help rounded-[1px]" title="Uso Parcial (30% - 74% reservado)" />
                <div className="flex-1 h-full bg-orange-500 dark:bg-orange-500 border border-black/10 dark:border-white/10 hover:scale-y-125 transition-all duration-150 origin-bottom cursor-help rounded-[1px]" title="Ocupação Elevada (75% - 89% reservado)" />
                <div className="flex-1 h-full bg-orange-600 dark:bg-orange-600 border border-black/10 dark:border-white/10 hover:scale-y-125 transition-all duration-150 origin-bottom cursor-help rounded-[1px]" title="Quase Cheio (90% - 99% reservado)" />
                <div className="flex-1 h-full bg-red-600 dark:bg-red-500 border border-black/10 dark:border-white/10 hover:scale-y-125 transition-all duration-150 origin-bottom cursor-help rounded-[1px]" title="Totalmente Esgotado (100%)" />
              </div>
            </div>
          </div>
          ...
          {weekDays.map((day, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={cn(
                  "px-2 sm:px-3 flex flex-col border-t-4 border-black dark:border-white relative z-10 bg-white dark:bg-zinc-950 justify-center h-[40px] sm:h-[50px] shrink-0"
                )}>
                    ...
                    {timeSlots.map((slot) => {
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            "flex-1 max-w-[16px] h-full relative cursor-crosshair",
                            color,
                            hoveredHeatmapInfo?.date === dateKey && hoveredHeatmapInfo?.slot === slot
                              ? "ring-2 ring-black dark:ring-white z-30"
