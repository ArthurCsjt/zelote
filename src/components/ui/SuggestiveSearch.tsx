...
export const SuggestiveSearch: React.FC<SuggestiveSearchProps> = ({
  ...
  const search = isControlled ? value : localSearch;
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const trailW = showTrailing
        ...
        : 0;
      const left = padLeft + leadW + (showLeading ? 8 : 0);
  ...
  return (
    ...
    >
      ...
      {showLeading && Leading && (
        <div ref={leadingRef} className="absolute left-3 top-1/2 -translate-y-1/2 flex-shrink-0 z-10">
          <Leading />
        </div>
      )}
      ...
      {showTrailing && Trailing && (
        <div ref={trailingRef} className="absolute right-3 top-1/2 -translate-y-1/2 flex-shrink-0 z-10">
          <Trailing />
        </div>
      )}
